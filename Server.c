...
#define MIRROR_IP "192.168.1.2" // IP address of the mirror server
...
int tar_directory_with_extensions(char *dirname, char *output_filename, char **extensions, int num_extensions);
void process_client(int client_socket);

int main(){

    int server_socket, client_socket, address_len;
    struct sockaddr_in server_address, client_address, mirror_addr;
    int opt = 1;
    ...
    while (1)
    {
        ...
        printf("Connected message sent to client.\n");

        count++;
        if (count % 4 == 0) {
            if (is_mirror) {
                printf("Switching back to server.\n");
                is_mirror = 0;
            } else {
        ...
        if (is_mirror) {
            ...
            send(mirror_fd, hello, strlen(hello), 0);
            printf("Connected message sent to mirror server.\n");
            close(mirror_fd);
        }
...
int tar_directory_with_extensions(char *dirname, char *output_filename, char **extensions, int num_extensions)
{
    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        {
            ret = tar_directory_with_extensions(filepath, output_filename, extensions, num_extensions);
            if (ret != 0)
            {
                return ret;
            }
